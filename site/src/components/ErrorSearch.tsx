  ...
  return (
    <div className="space-y-6">
      ...
      <div className="opacity-100 mt-6 transition-all duration-300 w-full max-w-full overflow-hidden">
        ...
        <div className="space-y-8 mt-8 animate-in fade-in slide-in-from-bottom duration-300">
          {/* Featured Categories & Top Technicians Grid */}
          <div className="bg-white rounded-3xl border border-slate-200 p-6 sm:p-8 space-y-6 shadow-2xs">
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 pb-4 border-b border-slate-100">
              <div className="text-right">
                <h3 className="font-extrabold text-sm sm:text-base text-slate-900 flex items-center gap-2 justify-start">
                  <Sparkles className="w-5 h-5 text-amber-500 animate-pulse" />
                  <span>دسته‌بندی‌های پیشنهادی و تکنسین‌های برتر کدیار۲۴</span>
                </h3>
                <p className="text-[11px] text-slate-500 mt-1">پیشنهاد برترین کارشناسان فنی کشور بر اساس ۱۸۰ روز گارانتی رسمی عملکرد</p>
              </div>
            </div>

            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5">
              {APPLIANCE_CATEGORIES.slice(0, 4).map((cat) => {
                const config = categoryConfig?.[cat] || {};
                const topTechId = config.topTechId;
                const topTech = topTechId ? technicians?.find(t => String(t.id) === String(topTechId)) : null;
                const errorCount = errorCodes.filter(err => err.category === cat).length;
                
                return (
                  <div key={cat} className="group bg-slate-50/50 hover:bg-white border border-slate-200/80 hover:border-blue-300 rounded-2xl p-5 transition-all duration-300 hover:shadow-md text-right flex flex-col justify-between h-full">
                    <div>
                      {/* Header */}
                      <div className="flex justify-between items-start mb-4">
                        <span className="bg-blue-50 text-blue-700 text-[10px] font-black px-2.5 py-1 rounded-xl border border-blue-100">
                          {errorCount} خطای ثبت‌شده
                        </span>
                        <div className="w-10 h-10 bg-gradient-to-br from-blue-500 to-indigo-600 rounded-2xl flex items-center justify-center text-white text-lg shadow-sm">
                          🛠️
                        </div>
                      </div>

                      <h4 className="font-black text-slate-900 text-sm mb-2 group-hover:text-blue-600 transition-colors">
                        عیب‌یابی تخصصی {cat}
                      </h4>
                      <p className="text-[10px] text-slate-500 leading-normal mb-5">
                        کدهای خطای کامل، نقشه‌های برد الکترونیک و راهنمای تعمیرات فوری {cat} سراسر کشور.
                      </p>
                    </div>
                    ...
                    <div className="border-t border-slate-100 pt-4 mt-2">
                      {topTech ? (
                        <div className="space-y-3">
                          <div className="flex items-center gap-2.5">
                            <img
                              src={topTech.avatarUrl || "data:image/svg+xml;utf8,<svg xmlns='http://www.w3.org/2000/svg' viewBox='0 0 100 100'><circle cx='50' cy='35' r='20' fill='%23ccc'/><path d='M20 85c0-15 15-25 30-25s30 10 30 25z' fill='%23ccc'/></svg>"}
                              alt={topTech.name}
                              referrerPolicy="no-referrer"
                              className="w-9 h-9 rounded-full object-cover border-2 border-white ring-2 ring-emerald-500/30"
                            />
                            <div className="flex-1 min-w-0">
                              <span className="font-extrabold text-[11px] text-slate-800 block truncate leading-tight">{topTech.name}</span>
                              <span className="text-[9px] text-emerald-600 font-extrabold flex items-center gap-0.5 mt-0.5">
                                ⭐ {topTech.rating || '5.0'} | {topTech.satisfactionRate || 98}% رضایت
                              </span>
                            </div>
                          </div>
                          <button
                            ...
                            className="w-full bg-slate-900 hover:bg-slate-800 text-white text-[10px] font-bold py-2 rounded-xl transition-all cursor-pointer text-center"
                          >
                            مشاهده کدهای خطا و رزرو {topTech.name}
                          </button>
                        </div>
                      ) : (
                        <div className="space-y-3">
                          <p className="text-[10px] text-slate-400 text-center font-bold">تکنسین برتر اختصاصی تعیین نشده است</p>
                          <button
                            type="button"
                            onClick={() => {
                              setSelectedCategory(cat);
                            ...
                            className="w-full bg-blue-600 hover:bg-blue-500 text-white text-[10px] font-bold py-2 rounded-xl transition-all cursor-pointer text-center"
                          >
                            مشاهده لیست کدهای خطا
                          </button>
                        </div>
                      )}
                    </div>
          ...
          {/* Affiliate Partner Products Section */}
              ...
              <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 pb-4 border-b border-slate-100">
                <div className="text-right">
                  <h3 className="font-extrabold text-sm sm:text-base text-slate-900 flex items-center gap-2 justify-start">
                    ...
                    <span>فروشگاه مرکزی و تامین قطعات یدکی همکاران (همکاری در فروش)</span>
                  </h3>
                  <p className="text-[11px] text-slate-500 mt-1">تامین مستقیم کلیه بردهای الکترونیکی و قطعات کمیاب با کمترین قیمت بازار</p>
                </div>
              </div>

              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
                {affiliateProducts.map((p) => (
                  <div key={p.id} className="group bg-slate-50/35 hover:bg-white border border-slate-200/80 hover:border-amber-300 rounded-2xl p-5 transition-all duration-300 hover:shadow-md flex flex-col justify-between text-right">
                    <div className="space-y-3">
                      <div className="relative w-full aspect-video rounded-xl overflow-hidden bg-white border border-slate-150 shrink-0 select-none">
                        <img
                          referrerPolicy="no-referrer"
                          src={p.image || "https://images.unsplash.com/photo-1581092160607-ee22621dd758?w=500"}
                          alt={p.name}
                          className="w-full h-full object-cover group-hover:scale-102 transition-all duration-300"
                        />
                        <div className="absolute top-2.5 right-2.5 flex gap-1">
                          <span className="bg-slate-900/80 text-white text-[9px] font-extrabold px-2 py-0.5 rounded-md backdrop-blur-xs">
                            {p.brand}
                          </span>
                          <span className="bg-blue-600/90 text-white text-[9px] font-extrabold px-2 py-0.5 rounded-md backdrop-blur-xs">
                            {p.category}
                          </span>
                        </div>
                      </div>
                      ...
                      <div className="space-y-1.5">
                        <h4 className="font-extrabold text-xs sm:text-sm text-slate-850 line-clamp-1">
                          {p.name}
                        </h4>
                        <p className="text-[10px] text-slate-450 font-mono leading-tight">
                          مناسب برای مدل: {p.model}
                        </p>
                    ...
                    <div className="mt-4 pt-3 border-t border-slate-100 flex items-center justify-between">
                      <div>
                        <span className="text-[9px] text-slate-400 block font-normal">قیمت قطعه همکار:</span>
                        <span className="font-black text-amber-600 text-xs sm:text-sm font-sans">
                      ...
                      <a
                        ...
                        className="bg-amber-500 hover:bg-amber-600 text-slate-900 hover:text-slate-950 text-[10px] sm:text-xs font-black px-4 py-2.5 rounded-xl cursor-pointer shadow-2xs hover:shadow transition-all inline-flex items-center gap-1.5"
                      >
